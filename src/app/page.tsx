...
// Historical chess move coordinates database
interface ChessMove {
  notation: string;
...
const LEGA_MATE_GAME: ChessMove[] = [
  { notation: "1. e4 e5", whiteMove: { from: [1, 4], to: [3, 4], piece: "♙" }, blackMove: { from: [6, 4], to: [4, 4], piece: "♟" }, description: "Traditional Open Game kickoff. Kings' pawns challenge the center." },
  { notation: "2. Nf3 d6", whiteMove: { from: [0, 6], to: [2, 5], piece: "♘" }, blackMove: { from: [6, 3], to: [5, 3], piece: "♟" }, description: "Philidor Defence. Black guards the e5 pawn with a modest pawn step." },
  { notation: "3. Bc4 Bg4", whiteMove: { from: [0, 5], to: [3, 2], piece: "♗" }, blackMove: { from: [7, 2], to: [4, 5], piece: "♝" }, description: "Shivika: 'An aggressive pin on our white knight, but white's lines are solid.'" },
  { notation: "4. Nc3 g6?!", whiteMove: { from: [0, 1], to: [2, 2], piece: "♘" }, blackMove: { from: [6, 6], to: [5, 6], piece: "♟" }, description: "Black prepares a fianchetto, neglecting active development." },
  { notation: "5. Nxe5! Bxd1??", whiteMove: { from: [2, 5], to: [4, 4], piece: "♘" }, blackMove: { from: [4, 5], to: [7, 3], piece: "♝" }, description: "Brilliant tactical sacrifice! White relinquishes the Queen for a forced mate." },
  { notation: "6. Bxf7+ Ke7", whiteMove: { from: [3, 2], to: [6, 5], piece: "♗" }, blackMove: { from: [7, 4], to: [6, 4], piece: "♚" }, description: "The King is flushed into the open under heavy fire." },
  { notation: "7. Nd5# Checkmate", whiteMove: { from: [2, 2], to: [3, 4], piece: "♘" }, description: "Légal's Mate! Complete tactical coordination in just 7 moves." },
];
...
export default function Home() {
  ...
  return (
    <div className="relative font-body bg-transparent text-white no-scrollbar selection:bg-gold selection:text-black">
      ...
      {/* MODALS AND PRELOADER FROM APP.TSX */}
              ...
              {!isSubmitted ? (
                <>
                  ...
                  <form onSubmit={handleBookingSubmit} className="w-full flex flex-col gap-4 text-left">
                    ...
                    <button
                      type="submit"
                      className="w-full bg-white hover:bg-gold text-black hover:text-white rounded-full py-3 mt-4 text-sm font-semibold transition-all duration-300 flex items-center justify-center gap-1.5 cursor-pointer hover:scale-[1.02] active:scale-95 shadow-lg shadow-black/40"
                    >
                ...
                >
                  ...
                  <button
                    onClick={resetBookingForm}
                    className="clay-btn hover:bg-white hover:text-black rounded-full px-8 py-2.5 text-xs font-semibold text-white cursor-pointer border border-white/15 transition-all"
                  >
                    Return to Academy
                  </button>
                </motion.div>
